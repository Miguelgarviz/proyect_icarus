...
export async function seedTestDatabase(
  ...
  for (let x = 0; x < 32; x++) {
    const externalId = stations.get(x) ?? orbit0ExternalIds[x];

    const tile = await prisma.tile.create({
      data: {
        externalId,
        type: stations.has(x) ? TileType.SPACE_STATION : TileType.EMPTY,
        positionX: x,
        positionY: 0,
        drillAttempts: 0,
        gameId: game.id,
  ...
  for (let x = 0; x < 16; x++) {
    const externalId = orbit1SpecialIds.get(x) ?? `orbit1_${x}`;
  ...
  if (!player1Tile || !player2Tile) {
    throw new Error('No se encontraron las Tiles iniciales de los jugadores.');
  }
