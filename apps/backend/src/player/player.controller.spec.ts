...
describe('PlayerController', () => {
  ...
  beforeAll(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [PlayerService, ShipService, StorageService, PrismaService],
    }).compile();
  ...
  describe('GET /player/:id', () => {
    ...
    it('should return a 404 if the player does not exist', async () => {
      await request(app.getHttpServer()).get(`/player/999999`).expect(404);
    });
    it('should handle a non-numeric id gracefully', async () => {
      // Number('abc') es NaN — Prisma recibirá NaN y fallará
      await request(app.getHttpServer()).get('/player/abc').expect(400);
    });
  });
  ...
  describe('GET /lobby/:lobbyId', () => {
    ...
    it('should return a 404 if the lobby does not exist', async () => {
      await request(app.getHttpServer()).get(`/player/lobby/99999`).expect(404);
    });
  });
  ...
  describe('GET /:id/ship', () => {
    ...
    it('should return a 404 if the player or the ship does not exist', async () => {
      await request(app.getHttpServer()).get(`/player/9999/ship`).expect(404);
  ...
  describe('POST /player/:lobbyId/ship', () => {
    ...
    it('should return a 404 if the lobby does not exist', async () => {
      await request(app.getHttpServer()).post(`/player/9999/ship`).expect(404);
    });
  });
  ...
  describe('DELETE /player/:id', () => {
    ...
    it('should return a 404 if the player does not exist', async () => {
      await request(app.getHttpServer()).delete(`/player/9999`).expect(404);
    });
  });
});
