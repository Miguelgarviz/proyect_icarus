...
export class GameService {
  ...
  haveAchivedGoal(lobby: Lobby, storage: Storage) {
  ...
  async movePlayer(tile: Tile, actualPlayer: Player, ship: Ship) {
    ...
    const originTile = await this.tileService.getTilesByCoordinates(
      tile.gameId!,
      ship.positionX,
      ship.positionY,
    );
    ...
    if (tile.positionY === ship.positionY) {
      ...
      distance =
        distance +
        (await this.calculateNumberOfPlayersBetween(
          start,
          end,
          tile.gameId!,
          false,
      ...
      for (const id of landingTilesId) {
        const possibleTile = await this.tileService.getTileByExternalId(
          id,
          tile.gameId!,
        );
      ...
      if (connectionTileExternalId) {
        const startTile = await this.tileService.getTileByExternalId(
          connectionTileExternalId,
          tile.gameId!,
        );
        const start = { x: startTile.positionX, y: startTile.positionY };
        const end = { x: tile.positionX, y: tile.positionY };

        distance = 1 + this.calculateDistance(start, end);
        distance =
          distance +
          (await this.calculateNumberOfPlayersBetween(
            start,
  ...
  async calculateNumberOfPlayersBetween(
    ...
    if (start.x === end.x) {
      if (checkStartTile) {
        const targetTile = await this.tileService.getTilesByCoordinates(
          gameId,
          start.x,
          start.y,
        );
  ...
  async calculateMaxDistance(player: Player, ship: Ship, gameId: number) {
    ...
    for (const x of validXPositions) {
      const tile = await this.tileService.getTilesByCoordinates(
        gameId,
        x,
        ship.positionY,
      );
    ...
    if (ship.externalId.includes('space_station')) {
      ...
      for (const landingTileId of landingTilesId) {
        ...
        const movementAfterLanding =
          remainingMovement - (isLandingOccupied ? 1 : 0);
        ...
        const rawDir4 =
          (tile.positionX + movementAfterLanding) % targetMaxPositions;

        const obstaclesDir3 = await this.calculateNumberOfPlayersBetween(
          { x: tile.positionX, y: tileY },
          { x: rawDir3, y: tileY },
          gameId,
          false,
        );
        const obstaclesDir4 = await this.calculateNumberOfPlayersBetween(
          { x: tile.positionX, y: tileY },
          { x: rawDir4, y: tileY },
          gameId,
          false,
        ...
        const dir3 = (rawDir3 + obstaclesDir3) % targetMaxPositions;
        ...
        while (currentX2 !== dir4) {
          if (currentX2 !== tile.positionX || !isLandingOccupied)
            subTilesToQuery.push(currentX2);
          currentX2 = (currentX2 + 1) % targetMaxPositions;
        }
        if (dir4 !== tile.positionX || !isLandingOccupied)
          subTilesToQuery.push(dir4);

        for (const numTile2 of subTilesToQuery) {
          const otherTile = await this.tileService.getTilesByCoordinates(
            gameId,
            numTile2,
            tileY,
          );
          if (otherTile && !reachableTiles.some((t) => t.id === otherTile.id))
