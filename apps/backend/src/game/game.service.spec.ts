...
import { Game, Prisma, Dificulty, Player, Tile, Ship, Storage, Lobby } from '../generated/prisma/client';
...
describe('GameService', () => {
  ...
  it('should create a new game', async () => {
    ...
    const gameInDb = await prisma.game.findUnique({ where: { id: result.id } });
  ...
  describe('updateGame', () => {
    ...
    it('should throw Prisma error if the game does not exist', async () => {
      await expect(service.updateGame({ data: updateData, where: { id: 999999 } })).rejects.toMatchObject({
        code: 'P2025',
      });
    });
  });
  ...
  describe('setGameStore', () => {
    ...
    it('should throw Prisma error if the game does not exist', async () => {
      await expect(service.setGameStore({ id: 99999999}, testData.store2.id)).rejects.toMatchObject({
        code: 'P2025',
      });
    });
  });
  ...
  describe('nextPlayer', () => {
    it('should reset player movement to ship engine value', async () => {
      ...
      const updatedPlayer = await prisma.player.findUniqueOrThrow({ where: { id: testData.player2.id } });
      const updatedShip = await prisma.ship.findUniqueOrThrow({ where: { id: testData.ship2.id } });
      const updatedGame = await prisma.game.findUniqueOrThrow({ where: { id: testData.game.id } });
  ...
  describe('upgradeShield', () => {
    it('should increase ship shield by one', async () => {
      const ship = await prisma.ship.findUniqueOrThrow({ where: { id: testPlayer.shipId! }});
      const storage = await prisma.storage.findUniqueOrThrow({ where: { id: testPlayer.storageId! } });

      await service.upgradeShield(ship, storage);

      const updatedShip = await prisma.ship.findUniqueOrThrow({ where: { id: ship.id } });
      const updatedStorage = await prisma.storage.findUniqueOrThrow({ where: { id: storage.id } });
  ...
  describe('increaseSupernovaLvL', () => {
    it('should increase supernova level by one', async () => {
      ...
      const updatedGame = await prisma.game.findUniqueOrThrow({where: {id: createdGame.id } });
  ...
  describe('haveAchivedGoal', () => {
    it('should return true when all required resources are reached', async () => {
      const storage = await prisma.storage.findUniqueOrThrow({where: {id: testData.player1.storageId!} })
      const goal = await service.haveAchivedGoal(testData.lobby1, storage)
    ...
    it('should return false when green requirement is not met', async () => {
      const storage = await prisma.storage.findUniqueOrThrow({where: {id: testData.player3.storageId!} })
      const goal = await service.haveAchivedGoal(testData.lobby2, storage)
    ...
    it('should return true when requirement is are met with surpluses', async () => {
      const storage = await prisma.storage.findUniqueOrThrow({where: {id: testData.player2.storageId!} })
      const goal = await service.haveAchivedGoal(testData.lobby1, storage)
  ...
  describe('movePlayer', () => {
    it('valid movement in the same orbit without other players in the way', async () => {
      const tile = await tileService.getTilesByCoordinates(testData.game.id, 0,0)
      await service.movePlayer(tile, testData.player1, testData.ship1)

      const player = await prisma.player.findUniqueOrThrow({ where: {id: testData.player1.id } });
      const ship = await prisma.ship.findUniqueOrThrow({ where: { id: player.shipId! }});
      const startTile = await tileService.getTilesByCoordinates(testData.game.id, 2,0);
      const endTile = await tileService.getTilesByCoordinates(testData.game.id, 0,0)
    ...
    it('valid movement in diferent orbits with other players in the way', async () => {
      const tile = await tileService.getTilesByCoordinates(testData.game.id, 1,1)
      
      const pl = await prisma.player.findUniqueOrThrow({ where: {id: testData.player1.id } });
      const sh = await prisma.ship.findUniqueOrThrow({ where: { id: pl.shipId! }});

      await service.movePlayer(tile, pl, sh)

      const player = await prisma.player.findUniqueOrThrow({ where: {id: testData.player1.id } });
      const ship = await prisma.ship.findUniqueOrThrow({ where: { id: player.shipId! }});
      const startTile = await tileService.getTilesByCoordinates(testData.game.id, 0,0);
      const landingTile = await tileService.getTilesByCoordinates(testData.game.id, 0,1)
      const endTile = await tileService.getTilesByCoordinates(testData.game.id, 1,1)
    ...
    it('valid movement in same orbit with other players in the way and backwards', async () => {
      const tile = await tileService.getTilesByCoordinates(testData.game.id, 15,1)
      ...
      const player = await prisma.player.findUniqueOrThrow({ where: {id: testData.player1.id } });
      const ship = await prisma.ship.findUniqueOrThrow({ where: { id: player.shipId! }});
      const startTile = await tileService.getTilesByCoordinates(testData.game.id, 1,1);
      const endTile = await tileService.getTilesByCoordinates(testData.game.id, 15,1);
    ...
    it('invalid movement in same orbit without enough movement points', async () => {
      const tile = await tileService.getTilesByCoordinates(testData.game.id, 12,1)
      ...
      const player = await prisma.player.findUniqueOrThrow({ where: {id: testData.player1.id } });
      const ship = await prisma.ship.findUniqueOrThrow({ where: { id: player.shipId! }});
      const startTile = await tileService.getTilesByCoordinates(testData.game.id, 15,1);
      const endTile = await tileService.getTilesByCoordinates(testData.game.id, 12,1);
    ...
    it('invalid movement if other player is in the same tile', async () => {
      const tile = await tileService.getTilesByCoordinates(testData.game.id, 0,1)
      ...
      const ship = await prisma.ship.findUniqueOrThrow({ where: { id: player.shipId! }});
      const startTile = await tileService.getTilesByCoordinates(testData.game.id, 15,1);
      const endTile = await tileService.getTilesByCoordinates(testData.game.id, 0,1);
