...
export class CardService {
  ...
  async createCardsForStore(
    storeId: number,
    numCards: number,
    cardsData: { type: CardType; cost: number },
  ) {
  ...
  shuffleCardsWithSeed(cards: Card[], seed: number): Card[] {
  ...
  async applyEnhancedScannerCard(drillCard: DrillCard, storage: Storage) {
    await this.prisma.storage.update({
      ...
      data: {
        green:
          storage.green + drillCard.greenResources <= 18
            ? { increment: drillCard.greenResources }
            : 18,
        red:
          storage.red + drillCard.redResources <= 10
            ? { increment: drillCard.redResources }
            : 10,
        yellow:
          storage.yellow + drillCard.yellowResources <= 10
            ? { increment: drillCard.yellowResources }
            : 10,
      },
    });
  }

  async applySlingShotCard(
    myShip: Ship,
    otherShip: Ship,
    player: Player,
    otherPlayer: Player,
