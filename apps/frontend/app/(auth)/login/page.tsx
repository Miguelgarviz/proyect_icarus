  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Guardamos el token para usarlo en las peticiones posteriores
        localStorage.setItem("access_token", data.access_token);
        router.push(`/home`);
      } else if (response.status === 401) {
        setError("IDENTIFICACIÓN FALLIDA — CREDENCIALES INCORRECTAS");
      } else {
      ...
      <div className="relative z-10 w-full max-w-sm px-6">
        ...
        <div className="mb-10 text-center">
          ...
          <h1 className="text-3xl font-black tracking-widest text-white uppercase">
            <span className="text-cyan-500">&gt;</span> Acceso
          </h1>
